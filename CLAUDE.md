...
- **Repository layer** (`src/lib/repo/`) — the single seam over persistence. `types.ts` defines the typed (synchronous) `Repository` contract; `localRepo.ts` implements it over `localStorage` and owns **all** `localStorage` keys + defensive parsing (freeze-stripping, `normalizeRestDays`, defaults). `index.ts` exports the active `repo`, which is now **`syncedRepo`** (`syncedRepo.ts`): localStorage for all reads/writes (the offline cache) **plus** an optional write-through **sink** that `SyncProvider` installs while signed in, which fans `saveDayRecords`/`savePrefs` out to Supabase in the background. `supabaseRepo.ts` implements the async contract against Supabase (day_logs + profiles.prefs; other domains delegate to localRepo — Phase 3). `PRSection` is the one holdout not yet behind the repo (see below).
...
- **Server-state scaffold** — the app is wrapped in TanStack Query's `QueryClientProvider` (`src/app/Providers.jsx`, mounted in `main.jsx`; shared client in `src/lib/queryClient.ts`). The first (and so far only) real consumers are the **friends** hooks in `src/hooks/useFriends.ts` (`useFriends`/`useRedeemInvite`, key `["friends"]`). The `AsyncRepository` interface (the Promise-returning mirror of `Repository`) is implemented by `supabaseRepo.ts`; `asyncLocalRepo` (exported as `asyncRepo`) is a Promise wrapper over `localRepo` for offline-first Query work.
- **Auth + cloud sync (Phase 1)** — `src/lib/supabase.ts` exports a **null-safe** client: when `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` are absent, `supabase` is `null`, `isSupabaseConfigured` is false, and the whole feature is dormant — the app is the byte-for-byte offline app (anonymous-first). `AuthProvider` (`src/providers/`) exposes `user`/`signInGoogle`/`signInEmail`/`signUpEmail`/`signOut`; `SyncProvider` reacts to auth: on first login it collects a username (onboarding), migrates local → Supabase, pulls + merges remote (per-day last-write-wins), installs the write-through sink, and **bumps a remount key** (`<div key=… style="display:contents">`) so the untouched hooks re-hydrate from the merged localStorage. Both mount in `Providers.jsx` inside the Query provider. Routing is now `react-router-dom` in `main.jsx` (`/` = app, `/auth/callback` = OAuth return). The **only** auth UI is `SignIn.jsx` in the Settings panel (self-contained like `PRSection`). SQL schema + RLS + the `redeem_invite` RPC live in `supabase/migrations/`; provisioning steps are in `SUPABASE_SETUP.md`. Denormalized `profiles.current_streak` is recomputed from the raw log via `computeState` on every sync.
- **Friends + invite (Phase 2, part 1)** — the friend graph over the Phase-1 plumbing. `src/lib/friends.ts` holds the Supabase I/O (`listFriends`, `redeemInvite` over the `redeem_invite` RPC); the pure, unit-tested helpers (`inviteUrl`, `mapRedeemError`) live in `src/lib/inviteCode.ts` (kept Vite/Supabase-import-free so they run under `node --test`, like `streakEngine.ts`). `useFriends.ts` wraps them in TanStack Query. UI: `FriendsOverlay.jsx` (bottom sheet like `BlockDatesOverlay`; invite QR via `InviteQR.jsx` + the `qrcode` dep, add-a-friend, list with each friend's `@username` + streak), opened from an AppShell button. The `/add/:code` route (`src/routes/AddFriend.jsx`) redeems a shared invite; when signed out it stashes the code in `nudge_pending_invite` (`src/lib/pendingInvite.ts`) and `SyncProvider` auto-redeems it once synced, then evicts the `["friends"]` cache on sign-out so it can't leak to the next user. **Leaderboard, unfriend, and the recompute Edge Function are still Phase 2/later** (tables exist, UI does not).
...
localStorage keys (note the legacy `fitness_` prefix from the app's original name; newer keys use `nudge_`) are centralized in `src/lib/repo/localRepo.ts` (`KEYS`), except a few self-contained keys: the PR section's `nudge_pr_cards_v1` and the transient, online-only `nudge_pending_invite` (a one-shot invite code awaiting sign-in, owned by `src/lib/pendingInvite.ts`):
`fitness_day_records`, `fitness_workouts_by_weekday`, `fitness_exercise_completion`, `fitness_streak_prefs`, `fitness_theme`, `nudge_pr_cards_v1`, `nudge_pending_invite`.
